...
const OrdersDashboard = () => {
  ...
  const fetchAllOrders = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:2008/api/orders");
  ...
  const fetchAllDealers = async () => {
    try {
      const res = await axios.get("http://localhost:2008/api/dealers");
  ...
  const fetchDealerOrders = async (dealerId) => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:2008/api/orders/dealer/${dealerId}`);
