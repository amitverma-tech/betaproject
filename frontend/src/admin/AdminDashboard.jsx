...
function AdminDashboard() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("http://localhost:2008/api/admin/agents")
      .then((res) => {
        setAgents(res.data || []);
        setLoading(false);
      })
  ...
  const loadAccessRequests = () => {
    axios
      .get("http://localhost:2008/api/access/requests")
  ...
  const loadAgentData = (agentId, type) => {
    ...
    axios
      .get(`http://localhost:2008/api/admin/agents/${agentId}/details`)
      .then((res) => {
  ...
  const handleAllow = (requestId) => {
    axios
      .post("http://localhost:2008/api/access/allow", { requestId })
      .then(() => {
  ...
  const handleReject = (requestId) => {
    axios
      .post("http://localhost:2008/api/access/reject", { requestId })
      .then(() =>
        setAccessRequests((prev) => prev.filter((r) => r._id !== requestId))
      )
      .catch((err) => console.log(err));
...
const BASE_URL = "http://localhost:2008";
...
farmer.photo
        ...
        if (farmer.ponds && farmer.ponds.length > 0) {
          farmer.ponds.forEach(pond => {
            ...
            const pondImageLink = pond.pondImage
              ? `http://localhost:2008/uploads/${pond.pondImage}`
              : "";
  ...
  const openHistoryModal = (itemId, type, name) => {
    ...
    axios
      .get(`http://localhost:2008/api/history/${type}/${itemId}`)
      .then((res) => {
...
<img
  src={`http://localhost:2008/${item[key]}`}
  alt={label}
  onClick={() =>
    openModal(`http://localhost:2008/${item[key]}`)
  }
  onError={(e) => {
          ...
          {agents.map((a) => (
            <tr key={a._id}>
              <td>
                {a.profilePic && (
                  <img
                    src={`http://localhost:2008/api/images/${a._id}/profile`}
                    alt="Profile"
                    onClick={() =>
                      openModal(`http://localhost:2008/api/images/${a._id}/profile`)
                    }
                  />
                )}
              </td>
              ...
              <td>
                {a.aadharFront && (
                  <img
                    src={`http://localhost:2008/api/images/${a._id}/aadharFront`}
                    alt="Aadhar Front"
                    onClick={() =>
                      openModal(
                        `http://localhost:2008/api/images/${a._id}/aadharFront`
                      )
                    }
                  />
                )}
              ...
              <td>
                {a.aadharBack && (
                  <img
                    src={`http://localhost:2008/api/images/${a._id}/aadharBack`}
                    alt="Aadhar Back"
                    onClick={() =>
                      openModal(
                        `http://localhost:2008/api/images/${a._id}/aadharBack`
                      )
                    }
                  />
                )}
              ...
              <td>
                {a.panCard && (
                  <img
                    src={`http://localhost:2008/api/images/${a._id}/pan`}
                    alt="PAN"
                    onClick={() =>
                      openModal(`http://localhost:2008/api/images/${a._id}/pan`)
                    }
                  />
                )}
              </td>
              ...
              <td>
                {a.savingAccountImage && (
                  <img
                    src={`http://localhost:2008/api/images/${a._id}/savingImg`}
                    alt="Passbook"
                    onClick={() =>
                      openModal(`http://localhost:2008/api/images/${a._id}/savingImg`)
                    }
                  />
                )}
              </td>
            ...
            <div className="document-images">
              {a.aadharFront && (
                <img
                  ...
                  className="doc-image"
                  onClick={() =>
                    openModal(`http://localhost:2008/api/images/${a._id}/aadharFront`)
                  }
                />
              )}
              {a.aadharBack && (
                <img
                  ...
                  className="doc-image"
                  onClick={() =>
                    openModal(`http://localhost:2008/api/images/${a._id}/aadharBack`)
                  }
                />
              )}
              {a.panCard && (
              ...
              <div className="pond-details-grid">
                {pondDetailFields.map(({ label, key, isClickableLink, isFileArray }) => {
                  ...
                  if (isClickableLink && key === "pondImage" && value) {
                    const imageUrl = `http://localhost:2008/uploads/${value}`;
                    return (
                    ...
                    return (
                      <div className="detail-row" key={key}>
                        ...
                        <div className="file-list">
                          {value.map((file, idx) => {
                            const fileUrl = `http://localhost:2008/uploads/${file}`;
                            return (
                ...
                onClick={() => {
                  ...
                  pondDetailFields.forEach(({ label, key }) => {
                    ...
                    if (key === "pondImage" && value) {
                      displayValue = `http://localhost:2008/uploads/${value}`;
                    }
                    
                    // Handle other file fields
                    if ((key === "pondFiles" || key === "fishFiles") && value) {
                      displayValue = Array.isArray(value) 
                        ? value.map(file => `http://localhost:2008/uploads/${file}`).join("; ")
                        : `http://localhost:2008/uploads/${value}`;
                    }
