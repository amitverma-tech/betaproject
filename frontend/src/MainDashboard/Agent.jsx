...
function Agent() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:2008/api/user")
      .then((res) => {
        ...
        const filteredUsers = res.data
          ...
          .map((u) => ({
            ...u,
            photo: u.photo ? `http://localhost:2008${u.photo}` : "/default-profile.png",
          }));
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:2008/api/access/approved-farmers/${agentId}`)
      .then((res) => {
  ...
  const openAgent = async (agentId, agentName) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:2008/api/farmers/by-agent?agentId=${agentId}&viewerId=${userId}`
      );
  ...
  const requestFarmerAccess = async (farmerId) => {
    ...
    try {
      const res = await axios.post("http://localhost:2008/api/access/request-farmer", {
        requesterId: userId,
        farmerId
      });
  ...
  const checkFarmerAccess = async (farmerId) => {
    try {
      const res = await axios.get(
        `http://localhost:2008/api/access/check-farmer?requesterId=${userId}&farmerId=${farmerId}`
      );
      ...
      if (res.data.approved) {
        const farmerRes = await axios.get(`http://localhost:2008/api/farmers/${farmerId}`);
  ...
  const loadNotifications = () => {
    ...
    axios
      .get(`http://localhost:2008/api/notification/${agentId}`)
      .then((res) => {
        const list = res.data || [];
  ...
  const markSeen = async (id) => {
    try {
      await axios.post("http://localhost:2008/api/notification/seen", { notiId: id });
        ...
        <div className="agent-user-section">
          ...
          <div className="agent-list-grid">
            {users.map((u) => (
              ...
              >
                <img
                  src={`http://localhost:2008/api/images/${u._id}/profile`}
                  alt={u.name}
